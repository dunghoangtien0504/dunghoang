...
export async function POST(req: NextRequest) {
  try {
    // Rate limit: chống spam yêu cầu rút tiền
    const rl = rateLimit(`aff-payout:${clientIp(req)}`, 5, 10 * 60_000)
    if (!rl.ok) {
      return NextResponse.json({ error: `Thử lại sau ${rl.retryAfter} giây` }, { status: 429, headers: { 'Retry-After': String(rl.retryAfter) } })
    }

    const { token, bank_account, bank_name } = await req.json()
    if (!token || !bank_account?.trim() || !bank_name?.trim()) {
      return NextResponse.json({ error: 'Thiếu thông tin' }, { status: 400 })
    }

    // Bắt buộc token có chữ ký — chống người lạ tạo yêu cầu rút bằng mã CTV đoán được
    const code = await verifyAffiliateToken(token)
    if (!code) {
      return NextResponse.json({ error: 'Token không hợp lệ. Dùng đúng link trong email CTV.' }, { status: 401 })
    }
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif;max-width:540px;margin:0 auto;color:#0D2B1A;background:#F6F0E4;padding:32px 16px;">
          ...
          <div style="background:#fff;border-radius:16px;padding:28px;border:1px solid #DDD8CB;">
            <p style="margin:0 0 12px;">Chào ${escapeHtml(aff.name)},</p>
            ...
            <p style="margin:0 0 4px;font-size:13px;color:#7A8C7E;">Chuyển vào: <strong>${escapeHtml(bank_name.trim())}</strong> — <strong>${escapeHtml(bank_account.trim())}</strong></p>
    ...
    await sendEmail({
      ...
      html:    `<p>CTV: ${escapeHtml(aff.name)} (${escapeHtml(aff.email)})<br/>Mã: ${escapeHtml(aff.ref_code)}<br/>Gross: ${gross.toLocaleString('vi-VN')}đ → Net: ${net.toLocaleString('vi-VN')}đ${taxable ? ` (khấu trừ thuế ${tax.toLocaleString('vi-VN')}đ)` : ''}<br/>Bank: ${escapeHtml(bank_name.trim())} — ${escapeHtml(bank_account.trim())}</p>`,
    })
